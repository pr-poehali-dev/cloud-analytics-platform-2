...
def handler(event: dict, context) -> dict:
    """Логин Telegram user-аккаунта №1 (для сторис). Управление сессией tg_user_session."""
    ...
    try:
        if method == 'GET':
            cur.execute("SELECT id, phone, logged_in, user_info, updated_at FROM tg_user_session WHERE id = 1")
            row = cur.fetchone()
        ...
        if action == 'send_code':
            ...
            cur.execute("UPDATE tg_user_session SET phone=%s, phone_code_hash=%s, logged_in=false, updated_at=%s WHERE id=1",
        ...
        if action == 'confirm_code':
            ...
            cur.execute("SELECT phone_code_hash FROM tg_user_session WHERE id=1")
            row = cur.fetchone()
            ...
            cur.execute("UPDATE tg_user_session SET session_string=%s, logged_in=true, user_info=%s, updated_at=%s WHERE id=1",
        ...
        if action == 'logout':
            cur.execute("UPDATE tg_user_session SET session_string=NULL, logged_in=false, updated_at=%s WHERE id=1", (datetime.now(),))
            conn.commit()
